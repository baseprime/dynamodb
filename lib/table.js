...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  self.log.info({params : params}, 'dynamo %s request', method.toUpperCase());
  driver[method].call(driver, params, function (err, data) {
    var elapsed = Date.now() - startTime;

    if (err) {
      self.log.warn({err : err}, 'dynamo %s error', method.toUpperCase());
      return callback(err);
    } else {
      self.log.info({data : data}, 'dynamo %s response - %sms', method.toUpperCase(), elapsed);
